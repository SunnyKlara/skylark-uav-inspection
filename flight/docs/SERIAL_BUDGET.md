...
### 测量口径：`UXRCE_DDS_SYNCT=1`（出厂值）

本节数字是在 `UXRCE_DDS_SYNCT=1`（出厂）下测的，即**时间戳同步开启**。
之所以要点明：后来为根治 offboard 自发掉线，SITL 的动作测试改成了 `SYNCT=0`
（见 `OFFBOARD_CONSTRAINTS.md` §7.2）。两种口径在链路上的差别是
`timesync_status` 这条话题及其背后的同步交互。

影响很小但要说清：`timesync_status` 实测 48 B × 0.99 Hz = **48 B/s**，
占实测总量的 0.1%。关掉同步后带宽略降，不改变「占裸容量 52%」的结论。

**若真机最终决定关 `SYNCT`**，串口预算的构成会变（少这条话题、少同步往返），
届时应按 §6 的方法重测一版，而不是沿用本节数字。
