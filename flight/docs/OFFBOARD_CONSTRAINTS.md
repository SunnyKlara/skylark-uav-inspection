...
即每 30 s 变化约 2.5 s，而 PX4 每 30 s 才校正一次。偏移一旦陈旧到超过
`COM_OF_LOSS_T`，入站 setpoint 被判过期 → 信号丢失 → 下次校正后恢复。

> ⚠ **本节曾把这组数据解释成「仿真时钟比墙钟慢约 8%」，方向和性质都错了，已撤回。**
> 更正见 §7.4：偏移是**变大**（不是变小），即仿真时钟相对墙钟**走快**；
> 而且它不是匀速漂移，是**离散前跳**。用无竞态的采集重测后，
> 平稳期的速率其实只差 0.3%（`99_notes/fp8`）。
> 当时的错误源于两点：只看偏移量的符号而没定义清楚正负方向，
> 以及采集侧存在并发竞态（见 §7.4）导致的假数据。

**关于「抬高 `COM_OF_LOSS_T` 有没有用」，此处的旧结论也要修正。**
原文说抬容限只是把边界往后挪、设 3.0 s 仍被 `AUTO.RTL` 接管 —— 现象没错，
但归因错了：那一轮我们**自己填的出站时间戳就是错的**（口径没跟随 `SYNCT`），
所以那不是对容限的公平检验。按 §7.4 的定因，容限设在观测到的跳变幅度之上是
**恰当的仿真侧补偿**；SITL 测试脚本现在设 5.0 s，并在读回断言里写明。
...
### 7.4 定因（2026-07-31，FollowPath 集成测试期间）

`SYNCT=0` 之后现象并没有消失：解锁飞行中跑一条 40 m×3 行的割草机航线，
每轮**恰好一次** `offboard_control_signal_lost`（持续 0.00 s，即单帧），
飞控随即切 `AUTO.RTL` 接管，动作以 `ABORTED_BY_FAILSAFE` 结束。
原始数据 `99_notes/fp5`…`fp10`。

飞控那边的判据只有一处（`~/PX4-Autopilot` v1.17.0
`src/modules/commander/HealthAndArmingChecks/checks/offboardCheck.cpp:46`）：
...
比较双方都在 PX4 时钟域，所以发送端必须用**飞控的刻度**写时间戳。
围绕这一条按顺序排除，每一步都有实测：

| 候选 | 实测 | 结论 |
|---|---|---|
| 发布端卡顿 | 心跳最大间隔 101~148 ms（名义 10 Hz） | 排除 |
| 入站断流（出站时间戳锚在入站帧上） | 位置消息最大间隔 22~57 ms | 排除 |
| 出站时间戳算错 | 平稳期滞后 **−45 ~ +7 ms**，即容限的 0.5%~0.7%（fp8/fp9 两轮独立测量） | 排除 |
| `offboardCheck` 的另一条路径 | 抓翻转瞬间的完整标志位快照：只有 `offboard/gcs/rc` 三项，无任何 `*_invalid` | 排除 |
| 仿真时钟离散前跳 | **相邻两帧位置消息之间，本机走 7 ms、飞控走 1936 ms**（fp9） | **成立** |

最后一项是关键：单次前跳 **1.93 s** 就已超过出厂容限 1.0 s。
跳变发生在飞控自己的时钟上，发送端无从预知，**任何由发送端填写的时间戳
都会在那一瞬间被判过期**。所以这是仿真保真度问题，不是实现缺陷。

**顺带修掉一个自己的真 bug（这条比上面更值得记住）**：
节点用 `MultiThreadedExecutor` + `ReentrantCallbackGroup`，位置回调会被多线程
并发执行，而 `px4_link` 当时在回调里先写 `self._lpos_recv_wall` 再拿它去配对，
于是「A 线程的时间戳」会和「B 线程的接收时刻」配成一对。
trace 里因此出现相邻两帧本机间隔 **−2852 ms** 的样本，还把时钟速率估计带歪
（一度误报漂移 8%~13%，并算出「跳变 2.87 s」这种假结论）。
加锁 + 丢弃乱序帧之后重测，才有上表里那些干净数字。
**教训：给排障用的埋点本身也要按并发安全写，否则它会伪造证据。**

工具与复现：
- 出站时间戳的时钟伺服：`px4_link.px4_now_estimate_us` / `_px4_clock_rate_locked`
- 出事时自动落盘 50 Hz 时序：`px4_link.dump_timing_trace`（路径由 `SKYLARK_TRACE_OUT` 指定）
- 离线定因：`python3 flight/tools/analyze_timing_trace.py <trace.csv> [--loss-t 1.0]`
  它会先自查乱序（乱序就直接判定「结论不可信，先修采集」），
  再用相邻帧插值算出滞后，并把「平稳期」与「跨跳变」分开报

**SITL 的处置**：`COM_OF_LOSS_T = 5.0`（高于观测到的跳变幅度），
写在 `flight/sitl/test_follow_path.sh` 并在参数读回里断言。
⚠ 真机没有 lockstep，不该有这种跳变，**S2 必须重新评估，不要照搬**；
5 s 容限在真机上等于机载电脑失联后多盲飞 4 秒。

**未结的开放问题**：`fp8` 与 `fp9` 各有一次丢失事件，其 trace 窗口内**没有**抓到
超过 0.5 s 的跳变（fp8 最大跳变 23 ms，平稳期滞后 ≤ +5 ms；fp9 那次丢失发生在
已抓到的跳变之后约 30 s）。也就是说「跳变」能解释这个现象，但不能保证解释了
每一次。剩下最可能的方向是 ROS→PX4 方向的投递停顿（入站方向不受影响，
所以不是 agent 整体卡住）。下一步若要继续查，用 `MicroXRCEAgent -v 6`
逐条记录并与 trace 的时间轴对齐。**目前按已知边界处置，不假装已完全查清。**
